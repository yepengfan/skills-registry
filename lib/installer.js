...
function installSkill(name, registryDir, claudeDir) {
  ...
  const { getSkillType } = require('./discovery');
  const type = getSkillType(pkgDir);
  if (!type) {
    console.log(color.yellow(`Skill ${name} has no commands/ or SKILL.md to install`));
    return;
  }
  ...
  if (type === 'commands') {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        ...
        console.log(color.yellow(`Warning: ${skillDst} exists and is not a symlink, skipping`));
        return;
      }
    } catch { /* does not exist */ }

    fs.symlinkSync(pkgDir, skillDst);
    console.log(color.green(`  Skill ${name}: ${pkgDir} -> ${skillDst}`));
  }
}

// ── Agent Definition (`.claude/agents/`) ───────────────────

function writeAgentDefinition(name, fm, bodyParts, claudeDir) {
  if (!fm.color) return;
  ...
  if (Array.isArray(fm.tools) && fm.tools.length > 0) {
    fmLines.push(`tools: ${fm.tools.join(', ')}`);
  }
  if (fm.model) fmLines.push(`model: ${fm.model}`);
  fmLines.push(`color: ${fm.color}`);
  fmLines.push('---');

  const dst = path.join(agentsDir, `${name}.md`);
  fs.writeFileSync(dst, fmLines.join('\n') + '\n\n' + bodyParts.join('\n\n'));
  console.log(color.green(`  Agent definition ${name} -> ${dst}`));
}
...
function installAgent(name, registryDir, claudeDir, visited, onlyInstalledSubagents = false) {
  ...
  // Write agent definition to .claude/agents/ (for colored labels in Claude Code UI)
  writeAgentDefinition(name, fm, parts, claudeDir);
...
function uninstallSkill(name, registryDir, claudeDir) {
  ...
  const pkgDir = path.join(registryDir, 'skills', name);
  const discovery = require('./discovery');
  const type = discovery.getSkillType(pkgDir);
...
function uninstallAgent(name, registryDir, claudeDir) {
  ...
  if (firstLine.includes(`agent-registry-path: ${registryDir}/`)) {
    ...
    // Also remove agent definition from .claude/agents/ if present
    const agentDefDst = path.join(claudeDir, 'agents', `${name}.md`);
    if (fs.existsSync(agentDefDst)) {
      fs.unlinkSync(agentDefDst);
      console.log(color.yellow(`  Removed agent definition: ${name}`));
    }
  } else {
