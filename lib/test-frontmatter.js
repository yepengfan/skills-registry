...
function testColorValid() {
  console.log('\n--- testColorValid ---');
  for (const c of fm.VALID_COLORS) {
    const content =
      '---\nname: c\ndescription: C\nversion: 1.0.0\nauthor: Me\ncolor: ' + c + '\n---\n\nBody.\n';
    const data = fm.parseFrontmatter(content);
    let threw = false;
    try { fm.validate(data); } catch (e) { threw = true; }
    check('color \'' + c + '\' is valid', !threw);
  }
}

function testColorInvalid() {
  console.log('\n--- testColorInvalid ---');
  const content =
    '---\nname: bad\ndescription: Bad\nversion: 1.0.0\nauthor: Me\ncolor: rainbow\n---\n\nBody.\n';
  const data = fm.parseFrontmatter(content);
  let threw = false, errMsg = '';
  try { fm.validate(data); } catch (e) { threw = true; errMsg = e.message; }
  check('invalid color rejected', threw);
  check('error mentions color', errMsg.includes('color'));
}

function testColorOptional() {
  console.log('\n--- testColorOptional ---');
  const content =
  ...
  let threw = false;
  try { fm.validate(data); } catch (e) { threw = true; }
  check('no color field is valid', !threw);
}
